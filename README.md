...
研究の現状ベスト。H1 Donchian ブレイクアウト(long-only)を MQL5 化し、**MT5 実機(Axiory)+
実コストで検証して確定**。**5ペア(XAUUSD/USDJPY/EURJPY/AUDJPY/GBPJPY)・risk0.5%/pair で
実機補正・合算 月利+2.62% / 合成DD9.4% / 年+36%(2024-26)**。

- 本体: [`strategies/standalone/breakout_h1.mq5`](strategies/standalone/breakout_h1.mq5)
- **運用ドキュメント(必読)**: [`strategies/standalone/BREAKOUT_README.md`](strategies/standalone/BREAKOUT_README.md)
- ★ レジーム依存(2021-26の金高/円安トレンド)に注意。下記 JPY3 プルバック(平均回帰)と
  月次相関+0.00で無相関 → 合算でヘッジ可能。

---

## 🥈 堅実モデル: mtf_pullback v2 — MT5 スタンドアロン EA (JPY3 basket)

研究の前半到達点。M5 + MTF(H1/M15)の ZigZag/ダウ理論ベースの押し目戦略を **MetaTrader5 の
...
3. **パラメータ**(テスター/EA 入力欄は `//` コメントのラベルで表示される):
   ...
   | `UTCオフセット自動` (`InpAutoUtcOffset`) | **true(既定)** ← EET+DST 自動判定。設定不要(非EETブローカーのみ手動) |
