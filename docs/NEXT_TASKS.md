...
月利を 2.1%→**4.4%/DD10%(信頼値)** に2倍化。詳細 PROGRESS「E章」。次の検証/実装:
- [ ] **エクイティカーブ・オーバーレイの forward 検証**。`tools/regime_overlay.py` の eqMA K20 m0.5(または
  口座DD基準 DDthr4% m0.5=EA実装容易) を MT5 EA に組込み、別期間/実機フォワードで DD削減が再現するか。
  - これが本物なら BO の再レバが正当化され月+4.4%が現実に。**最重要かつ最も費用対効果が高い次手**。
- [ ] **二層オーバーレイ(月+8%)には手を出さない**。K,m単調感応=overfit。forward で単層が通ってから再検討。
- [ ] **再レバ時のレジーム集中リスクのストレステスト**。BO H1+H4 をDD10%まで上げると円高/金安急反転で
  同時逝き。2021-26に反転局面が無いのが盲点。合成円露出/金露出の上限 or 反転シグナルでの一括縮小を検討。
- [ ] **MT5: 採用ポートフォリオの再サイズ**。overlay検証OKなら BO_H1_long 0.44%/pair + BO_H4_long 0.33%/pair
  (+PB 0.11% 保険)へ。現行 breakout_h1.mq5 / mtf_pullback_v2.mq5 に overlay ロジックを追加。
