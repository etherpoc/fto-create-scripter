...
### ★ P0 (2026-06-14 自律セッション成果 — 要ユーザ判断)
月利を 2.1%→**4.4%/DD10%(信頼値)** に2倍化。詳細 PROGRESS「E章」。実装済/残タスク:
- [x] **エクイティカーブ・オーバーレイの in-sample 頑健性検証**(`tools/overlay_validate.py`)。年次一貫・
  シャッフル対照で本物(連続DD捉え)と確認。MT5実装版(暦日MA)も proven 版に肉薄。
- [x] **MT5 実装**(`breakout_h1.mq5` の `InpOverlay`)。口座エクイティ<60日MAで新規ロット半減。
- [x] **再レバ尾部/レジーム反転ストレステスト**(`tools/stress_test.py`)。悪月3連=DD-13.5%(枠突破)、
  overlay は損失の63%を full size で食らう(lag)=急反転の第一撃は防げないと判明。
- [ ] **★まず『段階運用 step1』を実機フォワード**: 採用5ペア・**risk0.5%のまま overlay ON**(再レバしない)で、
  実機の DD が 9.4→~6% に下がるかを数か月確認。これが overlay の真の forward 検証。**最優先・最低リスク**。
- [ ] **step1でDD低下を確認できたら『step2: 部分再レバ』**(risk 0.5→0.6-0.7%)。**一気に枠上限(0.44/0.33相当)へ行かない**。
- [ ] **二層オーバーレイ(月+8%)には手を出さない**(K,m単調感応=overfit)。
- [ ] mtf_pullback_v2.mq5 にも同じ overlay を移植(口座エクイティ基準なので共通化可能)。

### ★ P0 — durable に 6-8%/月を埋める唯一の残路: 価格外データ(金利差/キャリー)
価格由来エッジは trend + overlay + 再レバで搾り尽くした(天井 信頼値月+4.4%)。残るギャップは
**真に無相関の +EV 新エッジ = 金利差/キャリー**(JPYクロスのトレンドのファンダ engine そのもの)。
- [ ] **データ調達**: 日米欧の政策金利/2年債利回り(FRED 等、無料)、CFTC COT 建玉(週次)。**point-in-time厳守**
  (発表日でアライン、ルックアヘッド厳禁)。recorded tick(2021-26)と時刻整合。
- [ ] **金利差ストリーム**: USDJPY 等の金利差の方向/モメンタム/極値を特徴に、GBM(深層ネットでなく)で
  meta-label or 方向判定。**既存BOと無相関なら合算でDD圧縮→再レバ→月利上振れ**。金利差縮小はキャリー巻戻し
  警報=overlay以上のレジーム保護にもなる(二重の価値)。
- [ ] 統合前に「金利差だけで net+ WF か」を単体検証。ダメなら meta-label 用フィルタとしてのみ採用。

### 旧 (継続)
- [ ] **JPY3 basket / breakout_h1 を数か月フォワード運用**(ユーザ判断)。各チャート・risk0.5%。
