...
- ★ **ショートのスワップ(マイナスキャリー)コストが未モデル。** 特に円クロスのショート(=円ロング=円安期は逆キャリー)
  では実コストがショートの優位を削る可能性 → **実機検証で要確認**(longには無い非対称コスト)。
- DD10%スケールは in-sample DD前提。新レジームでのショートDD挙動は不確実。最適αも期間依存(OOS=1.0/IS=0.4)。
- それでも**価格内で見つかった初の本物の無相関補完**。FTOで全部上昇だったため見逃していた(2015-20のリスクオフが鍵)。

### ★ 採用構成の最終診断 + WF頑健性 (`tools/axiory_portfolio.py`)
**long-7 + short-7×0.4 + overlay**(DD10%スケール, long risk≈0.43%/short≈0.17%):
- **OOS(2015-20) +2.63%/月(DD9.2%) / IS(2021-26) +3.72%/月(DD10.0%)、月次Sharpe1.96、最悪月-6.41%、負け月31%。**
- **年別 2015→2026 の12年すべてプラス**(+13〜+62)=真OOS含め12年連続黒字=強い頑健性。
- WFニュアンス: 最適αは**期間依存**(OOSはα=1.0=フルショート, ISはα=0.4)。リスクオフ期はショート多めが良い。
  **固定α=0.4でも両期間とも強+**=「片方だけ」ではない。α=0.4は妥当な折衷。

### 6-8%目標との距離(DDフロンティア)
リターン/DDはサイズに概ね線形 → **DD10%でIS+3.72%(目標の53%) / DD15%で約+5.6% / DD~19%で約+7%(到達)**。
**「6-8%/月はこのlong+short構成で DD~18-20%なら到達可能」。安全DD10%では約半分(Sharpe2/全年+)** が正直な現在地。
long-only時代(月+2.3%@DD10%)から約1.6-2倍の前進。
...
- 次: **MT5実機でlong+shortを検証**(最優先) / 第2の無相関源探索(価格内は枯れ気味→金利差carry) / DD許容の判断。

成果物: `tools/axiory_direction.py` / `axiory_longshort.py` / `axiory_portfolio.py`。

---

## I: 新ロジック探索の打ち止め — 価格内エッジはトレンド一択 (2026-06-14)

「0から新汎用ロジック」を実Axiory 11年で探索。**クロスペア統計裁定(ペアトレード)**を検証
(`tools/axiory_pairs.py`): 高相関9ペアのlogスプレッドz-score逆張り。
- **全9組が両期間net負け**(合算 OOS-804/IS-839)、WR27-42%(低い=スプレッドが反転せずバンド突破→損切頻発)。
- breakout相関 -0.12(無相関だが)net負け=無価値。2レグのコストが薄いエッジを食う。リキッドFXペアトレの限界。

### ★ 価格データ内の機序探索の総括(11年実データの一貫した結論)
| 機序 | 結果 |
|---|---|
| トレンドフォロー(Donchian breakout) | ✅ **唯一生存・実OOS頑健** |
| ショート(同機序の裏=スリーブ) | ✅ 底上げ(新エッジではない) |
| 平均回帰(MTF pullback) | ✗ 実データnet負け |
| 流動性スイープ逆張り(LSR) | ✗ コインフリップ |
| 統計裁定(ペアトレード) | ✗ net負け |
| MA交差 | ✗ breakoutと相関0.78=冗長 |

→ **価格データから取れるエッジはトレンドフォロー一択。平均回帰は全形態でnet死亡。** これ以上の価格内探索は
同じ結論の量産になる見込み。**genuinelyに新しい無相関エッジの残路は「価格外データ(金利差/キャリー/COT建玉)」のみ**
(`tools/fetch_yields.py`/`carry_lab.py` 完成・ユーザDL待ち)、または ML での弱シグナル非線形合成(過去実績は薄い)。

### 補足: マルチTFトレンドも検証済(H1が最適)
同一Donchianを H1/H4/D1 で比較: H1 効率13.3(最良) / H4 corr0.66=冗長・弱 / **D1 corr0.21で無相関ぎみだが
11年240本の超低頻度・効率5.3で寄与小**。→ 異なる時間軸トレンドも有意な追加ストリームにならず。**H1 が価格内の最適解**。

成果物: `tools/axiory_pairs.py`。

---

## J: ★ USD系の訂正 — 「トレンドはJPYだけ」は誤り。USDショート・ブレイクが効く (2026-06-14)

ユーザの指摘「USD系でも利益を出す人がいる＝手法は存在する/スキャや他手法を試し切ってない」を受け再検証。
**I章の『価格内は枯れた』は早計だった。**

### スキャ(M5平均回帰)は決定的に死亡(コスト)
USD系 M5 zフェードは全構成・全ペアで巨大net負け(合算OOS-13665/IS-12120, 13万件)。WR45-50%(方向は当たる)
だが**M5の小値幅が往復1.5-2pipsコストに全て食われる**=構造的。リテール機械スキャは不成立(`tools/axiory_meanrev.py`)。

### ★ USDは「短期Donchian・ショート方向」のブレイクで効く(`tools/axiory_usdopt.py`)
- WFパラメータ最適化(OOSで最適化→IS評価): **EURUSD/AUDUSD/USDCAD の3/6が頑健**。共通の傾向=
  **en10-20(短い)・sma100・short/both方向**(JPY/金用の en30/long-only をUSDに当てていたのが誤りだった)。
- 単一設定 **en20/sma100/short** を全6 USDに: **OOS+643/IS+138(両期間+)、robust 4/6、本体BO7-longと相関0.09(無相関)**。
- USDは2021-26に一方向トレンドが無いので long-only は死ぬが、ショート(=USD強/2015-22のドル高)は効く。

### ★ USDショート・スリーブ追加で portfolio 改善・頑健化
| 構成(DD10%換算, overlay) | OOS%/月 | IS%/月 | 効率 |
|---|---|---|---|
| long7 | +1.30 | +2.40 | 24.7 |
| long7 + 0.4short7 | +2.54 | +3.69 | 41.9 |
| **+ 0.5×USDショート** | **+3.91** | **+4.03** | **53.9** |
- OOSが大きく伸びOOS/IS均衡(+3.9/+4.0)=**レジーム依存が減り頑健化**。現在地 **約+4%/月@DD10%**(DD~15-17%で6-8%射程)。

### 教訓(自戒)
- **「無い」と決めつけるのも片手落ち。** JPY用パラメータ・long-onlyを横展開して「USD負け」と結論したのは誤り。
  ペア群ごとに最適な方向/パラメータが違う(JPY/金=long-trend、USD=short-trend)。
- ただしスキャ(低TF)はコストで構造的に不成立、という別の正直さも同時に確認。USDショートも2015-22ドル高依存の
  regime betで、ドル安局面は未検証。**MT5実機・スワップ込みで要検証**。

成果物: `tools/axiory_meanrev.py` / `axiory_usdopt.py`。
...
## YouTube手法検証: エリオット第3波 EMA戦略 → 不採用 (2026-06-15)

`highlevefx-eriot.md` (YouTubeチャンネルの「聖杯級/ほぼ負けない3波エントリー」) の再現性を検証。
コアルール (20EMA×200EMAクロス→20EMAタッチ基準足→実体ブレイク、クロス後1発のみ) を
`tools/verify_eriot3.py` に機械化。実Axiory H4 (2015-2026, 11.3年, 12ペア, net込み)。

- **結論: エッジほぼゼロ・レジーム依存で再現性なし。投入非推奨。**
- 全12ペア best(RR1:2): net合計+41R だが **IS(2015-20)=−36R / OOS(2021-26)=+77R**。
  プラスは2021-26の一方向トレンド(USDJPY/XAU/EUR大相場)に丸乗り。両期間net+のrobust基準を満たさない。
- **「勝率9割/ほぼ負けない」は不成立** — 実測WRはRR次第で26〜52%。
- **gross(コスト0)でもEV/件は+0.03〜0.06Rのみ** = 構造固有エッジ無し。単なる薄いトレンドフォロードリフト。
- ペア別はnet+ 6 / net− 6 のコインフリップ。USDCAD/USDCHFは明確に負け。
- 裁量フィルタ(レンジ回避/RR選別/日足同期/分割利確)はコード化不能=検証外。素のエッジ0なので
  裁量で「聖杯級」になる根拠は乏しい(後知恵バイアスの温床)。
- 同型の `highlevefx-ab-logic.md` (AB手法/5分足) も同構造のため同じく薄いと推定 → **下記で実証済**。
- 比較: 採用済み `breakout_h1` はIS/OOS両方net+の実証あり。本手法はそれに及ばず。

---

## YouTube手法検証: AB手法 (5分足 EMA) → 不採用 (2026-06-15)

`highlevefx-ab-logic.md` (YouTube「AB手法」勝率9割超え) の再現性を検証。`tools/ab_verify.py` に機械化
(spec: `strategies/ab_method/spec.md`)。実Axiory **M5** (XAU/GBPJPY/EURJPY/USDJPY/EURUSD/GBPUSD, net込み, WF OOS<2021<IS)。
コア: ema20×ema200クロスで方向固定→200EMAタッチ待ち→直近K本ブレイク(斜めライン近似=Donchian-K)で成行、
SL=直近スイング/TP=N計算近似(RR*risk)。

- **結論: 「勝率9割」は数字トリックとして再現するが、利益は全く再現せず。エッジゼロ。不採用。**
- **素骨格**: RR1.0で合算WR**49%=コインフリップ**。全ペア・全RR・全設定でnet負け(exp≈-0.12〜-0.16R)。IS/OOS両方✗。
- **「勝率9割」の正体 = RR<1の数字トリック**: SL=構造(広)/TP=「小さい波N計算」(狭)で実質RR<1。
  低RR掃引で WR: RR0.7→58% / RR0.5→65% / **RR0.3→73%**(XAU76%) と機械的に上昇。さらに詰めれば9割域に届く。
  だが **expは全RRで一定して負け(≈-0.12R)** — 勝率が上がっても満額SL+コストで相殺。利益は再現しない。
- **裁量(綺麗さ/勢い/柔軟決済)を定義化して機械化** (EMA乖離/ema200傾き/ブレイク足実体/伸び切り回避/H4整列/建値BE):
  アブレーションで全フィルタ個別・全部入りを検証 → **トレード数は19,813→2,439に激減(厳選は機能)するが
  1トレードのexpは不変(-0.12R)**。BE決済はむしろ悪化。
- **教訓: 厳選は既存エッジを"増幅"できても、無いエッジを"創出"はできない。** 方向シグナルの素WRが49%
  (コインフリップ)である以上、どう厳選してもexpは0近傍のままコストで負ける。
- `eriot3` (H4/エリオット第3波) と完全に同型の結論。EMA系YouTube手法は裁量チェリーピックを除くと素EV≈0。
- 限界: 斜めライン=Donchian-K近似なのでエントリー"タイミング"は本家と差。ただし方向シグナル自体に
  エッジ無し(49%)が根本問題で、これは忠実に機械化済。

---

## 金利差(キャリー)ストリーム検証 → NO-GO (2026-06-16)

頭打ち打破の本線候補だった「価格外情報=金利差」を検証。FRED OECD月次10年債利回り**8通貨**を
`tools/fetch_yields.py`(Stooq無音失敗→FREDへ移行) / ブラウザDL / `tools/convert_yields.py` で
取得・変換し `tools/carry_lab.py` で go/no-go(両期間net+ かつ BO無相関) 判定。

- **結論: NO-GO。** 3方式すべて不採用(8通貨フル, GBP込み):
  - level(水準差): 月利+0.51%/P1+53.3%/**P2−20.0%(後半負け)**/**BO相関+0.46**(=同レジーム,焼き直し)
  - mom_k3: 月利−1.49%/相関−0.02(無相関だが期待値マイナス)
  - mom_k6: 月利−2.47%/両期間マイナス
- 「**稼ぐ信号(level)はBOと正相関で焼き直し、無相関の信号(mom)は稼げない**」二択。
  `carry_lab`の事前懸念(キャリー=円キャリー=BOが乗るレジームそのもの)が実測で確認。
  DD圧縮・再レバ用の新ストリームにはならない。
- 環境メモ: Claudeサンドボックスは外部ネット遮断 → 利回りDLはユーザ環境必須。urllib直DLは
  プロキシ/TLSでstall、curl.exe / ブラウザが確実。生CSVは変換後に削除(data/yields/に変換済保持)。
- → 残る現実的な伸び代は **ショートスリーブの実機検証**(IS+3.72%/月, MT5未実機)に一本化。

---

## ショートスリーブ 実exec検証 → NO-GO (2026-06-16)

最後の伸び代候補だったショートスリーブを実機前にオフライン検証 (`tools/verify_short_sleeve.py`)。
`breakout_h1.mq5` には既に `InpShortOnly` でショート経路が実装済 = 「未実装」でなく「未検証」だった。
EAの確定足ロジック(指標shift2..n+1, 約定=次足始値の成行)を忠実に写経し net 比較。

- **結論: NO-GO。** ショートスリーブの+3.72%/月は研究エンジン `run_bo_fast` の
  **同足Donchianレベル約定(=スリッページ0の理想ストップ)** が生んだ幻。
- 約定モデル別 sumR (7ペア11年, net):
  | モデル | LONG | SHORT |
  |--------|------|-------|
  | 逆指値 slip=0p(研究相当) | +482R | +124R(但IS=−47で直近赤字) |
  | 逆指値 slip=1p(現実的) | +334R | −19R |
  | 逆指値 slip=2p(金/JPY) | +185R | −162R |
  | **成行・次足(現EA実挙動)** | **+349R** | **−238R** |
- long+0.4short の効率は long-only と同等以下(13.6→13.25)=スリーブの上積み消滅。
- 機序: ショートは2021-26の歴史的円安(JPYペア上昇)で轢かれる。研究は良いフィルでそれを覆い隠していた。
  **ブレイク約定はフィルに極めて感応的**(レベルちょうど vs 次足始値で long+133R/short+362R 差)。
  → ロングの実機値もgrossより小さいと理解すべき([[mt5-edge-does-not-transfer]] の再現)。
- **運用判断: ショートスリーブは投入しない。long-only breakout を継続。**

## 執行改善(逆指値化)検証 → NO-GO / 現EAが既に上限 (2026-06-16)

「成行→逆指値でフィル改善(longで+133R)」を当初の次手に挙げたが、**先読みを除いて正直に検証したら逆だった。**
ユーザの指摘「H1確定前にM5でブレイクは見える」=正しく、逆指値ならintrabar約定できる。だが**逆指値は当足の
終値が出る前に約定する以上、現EAの『終値>SMA & レベル超で確定』という終値確認フィルタを捨てねばならない**。

約定モデル別 sumR (7ペア11年, net, `tools/verify_short_sleeve.py`):
  | モデル | LONG | SHORT |
  |--------|------|-------|
  | [幻]逆指値+終値確認 slip0 (研究=先読み) | +482R | +124R |
  | [現EA]成行・次足・終値確認 | **+349R** | −238R |
  | [正直]逆指値・終値確認なし slip0 | +74R | −423R |
  | [正直]逆指値・終値確認なし slip1p | −65R | −556R |

- 終値確認を外すと**ダマシ(ヒゲ抜け戻り)を大量に拾い**、好フィル(+133R)を損失増(+349→+74R)が上回る。
- **分解**: 終値確認フィルタの価値 ≈ +408R(巨大) / フィル改善の価値 ≈ +133R。だが両取りは先読みでしか不可能。
  現実は「intrabar約定 ⇔ 終値確認」のどちらか。確認を捨てる逆指値化は純損(−275R)。
- **∴ 現EAの『H1終値で確認してから成行』はバグではなくエッジの源泉**(ダマシ除去)。執行改善の余地は無い。

## FX内分散レバー検証 → NO-GO + CADJPY追加(タダ飯) (2026-06-16)

「新データ無しで分散」= Axiory15ペアの未使用分(USD/EUR/GBPブロック)を足してポート改善するか
(`tools/verify_diversify_fx.py`, 実EA約定 long-only net)。
- **NO-GO。** 非JPYは軒並み単体ネット負け(AUDUSD−101/EURGBP−80/NZDUSD−67/USDCHF−55/…)。
  ブレイクのエッジは**「JPYクロス+金」固有**でUSD/EUR/GBPブロックには存在しない。
  ポート効率 JPY7=11.4 → ALL15=5.1(希釈)。JPY×nonJPY 月次相関+0.37(分散材料にもならず)。
- **タダ飯1つ**: CADJPY(+40R, 効率1.3の健全な勝ちペア)が現バスケットから漏れていた。
  JPY7→JPY8(+CADJPY)で効率11.4→11.8, IS月+1.34→+1.39%。コストゼロ・同機序 → **ライブに足すべき**
  (ただし分散でなく同ベットの上乗せ)。

### 現在地サマリ(2026-06-16) — 5本線すべて検証完了
価格内エッジ・金利差・ショートヘッジ・執行改善・**FX内分散**の5本線をすべて検証し尽くし、
**robust に黒字なのは long-only H1 ブレイク(JPYクロス+金, 成行・終値確認)のみ。これが現実解の上限。**
月利の天井は約+2.6%(再レバで+4.4%)で6-8%目標には未達。エッジは「円安/リスクオン」一相関クラスタに固有。
**戦略アイデアは出尽くし、真の制約は『アイデア』でなく『別アセットの信頼データ』。**
- 他アセット(指数/コモディティ)は信頼できる無料ヒストリカルが無い=詰まり点。
- **唯一の例外=暗号資産(BTC/ETH)**: 取引所public dataが堅牢・大量・無料。トレンド適性高くFXと低相関の本命。
  Axioryが暗号CFDを扱うなら、スポットをプロキシに現ロジックのエッジ有無+相関を検証する価値大(次の最有力手)。
