...
- **運用判断: ショートスリーブは投入しない。long-only breakout を継続。** ブレイクをEAで取るなら
  成行でなく逆指値発注が本筋(longで+133R改善余地)だが、それでもショートは救えない。

### 現在地サマリ(2026-06-16)
価格内エッジ(トレンド一択)・金利差・ショートヘッジの3本線をすべて検証し尽くし、
**robust に黒字なのは long-only H1 ブレイクのみ**。月利の天井は約+2.6%(再レバで+4.4%)で6-8%目標には未達。
価格内/金利差の残路はほぼ尽きた。次の伸び代は別データ源(センチメント/フロー/オルタナ)か、執行改善(逆指値化)。
