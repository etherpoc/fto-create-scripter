...
### 現在地サマリ(2026-06-16) — 4本線すべて検証完了
価格内エッジ(トレンド一択)・金利差・ショートヘッジ・**執行改善**の4本線をすべて検証し尽くし、
**robust に黒字なのは long-only H1 ブレイク(成行・終値確認, +349R/11年)のみ。これが現実解の上限。**
月利の天井は約+2.6%(再レバで+4.4%)で6-8%目標には未達。**価格内・金利差・執行の残路は尽きた。**
唯一残る方向は「価格・金利の外の新情報」(センチメント/フロー/COT/オルタナ)=別データ源の導入のみ。
